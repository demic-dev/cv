...
JSON-Resume YAML using the EXACT same schema as `generic.yaml` (see [[resume-schema]]) so `cv.typ`
...
- `basics.summary` is kept COMMENTED in every variant (user explicitly said do NOT enable), matching
  `generic.yaml`'s rendered state; the commented text is role-tailored so it can be enabled later.
...
- SKILL CHIPS added per-variant from details.md (NOT in canonical, do not touch generic.yaml):
...
**Keep in sync:** any fact removed/corrected in details.md/generic.yaml must propagate to all four
