...
- **details.md is authoritative** over `cv.typ` when values conflict. Known conflicts as of 2026-07:
  ...
  Ghost Blog stars are "+54k" in details.md (older generic.yaml said "+50k").
- **Dates already resolved in generic.yaml** (more precise than details.md's rough ranges):
...
- **University Startup Challenge** added as an `awards` entry (First Place, 2025, team of 4 → became
  ...
  details.md and are unverified; do not add them to generic.yaml without user confirmation.
...
- **"Accomplishments" section (decided 2026-07-03):** the `awards` section holds non-award accomplishments
  ...
  `cv.typ`: `#alta-from-yaml("generic.yaml", labels: (awards: "Accomplishments"), preferences: (...))`.
